...
use ini::Ini;
// COMMET: tract's own ndarray (0.16 from tract 0.21.13 on), re-exported so
...
// COMMET: the model's bytes, so that a caller can load it with
...
pub const DEFAULT_MODEL: &[u8] = include_bytes!("../models/DeepFilterNet3_onnx.tar.gz");

impl Default for DfParams {
    #[allow(unreachable_code)]
    fn default() -> Self {
        // COMMET: the low-latency model (DeepFilterNet3_ll) is not vendored.
        #[cfg(feature = "default-model")]
        {
            log::debug!("Loading model DeepFilterNet3_onnx.tar.gz");
            DfParams::from_bytes(DEFAULT_MODEL).expect("Could not load model config")
        }
...
fn init_encoder_impl(
    ...
    let s = m.symbols.sym("S"); // COMMET: `symbol_table` in tract 0.21.4
